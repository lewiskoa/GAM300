...
std::unordered_map<std::uint64_t, ScriptRuntime::ScriptType>    ScriptRuntime::s_instances{};
std::unordered_map<std::uint64_t, Boom::EntityId>               ScriptRuntime::s_instanceEntity{};
...
void ScriptRuntime::Shutdown() {
    ...
    for (auto& [id, vtbl] : s_instances) if (vtbl.d) vtbl.d(id);
    s_instances.clear();
    s_instanceEntity.clear();
    s_types.clear();
}

void ScriptRuntime::RegisterType(const std::string& typeName, CreateFn c, UpdateFn u, DestroyFn d) {
    std::lock_guard<std::mutex> lock(s_mutex);
    s_types[typeName] = ScriptType{ c, u, d };
    if (s_hooks.Log) {
        const auto msg = std::string("[ScriptRuntime] Registered ") + typeName;
        s_hooks.Log(msg.c_str());
    }
}

std::uint64_t ScriptRuntime::NextId() {
    return s_nextId.fetch_add(1, std::memory_order_relaxed);
}

std::uint64_t ScriptRuntime::CreateInstance(const std::string& typeName, Boom::EntityId e) {
    std::lock_guard<std::mutex> lock(s_mutex);
    auto it = s_types.find(typeName);
    if (it == s_types.end()) {
        if (s_hooks.Log) {
            const auto msg = std::string("[ScriptRuntime] Unknown type: ") + typeName;
            s_hooks.Log(msg.c_str());
        }
        return 0;
    }
...
void ScriptRuntime::DestroyInstance(std::uint64_t id) {
    std::lock_guard<std::mutex> lock(s_mutex);
...
void ScriptRuntime::UpdateInstance(std::uint64_t id, float dt) {
    std::lock_guard<std::mutex> lock(s_mutex);
    if (auto it = s_instances.find(id); it != s_instances.end()) {
