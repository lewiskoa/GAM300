...
{
    template<>
    struct var_type<glm::vec2> : var_defaults<"vec2", glm::vec2>
    {
        constexpr static inline char ctype_v = '2';
        constexpr static inline auto serialization_type_v = "2";
        template< typename T, auto CRC_V >
        constexpr static auto XCoreTextFile(T& FileStream, ::xproperty::any& Any)
        {
            if (Any.m_pType == nullptr || Any.m_pType->m_GUID != guid_v) Any.Reset<glm::vec2>();
            return FileStream.Field(CRC_V, "Value:?", Any.get<glm::vec2>());
        }
    };

    template<>
    struct var_type<glm::vec3> : var_defaults<"vec3", glm::vec3>
    {
        constexpr static inline char ctype_v = '3';
        constexpr static inline auto serialization_type_v = "3";
        template< typename T, auto CRC_V >
        constexpr static auto XCoreTextFile(T& FileStream, ::xproperty::any& Any)
        {
            if (Any.m_pType == nullptr || Any.m_pType->m_GUID != guid_v) Any.Reset<glm::vec3>();
            return FileStream.Field(CRC_V, "Value:?", Any.get<glm::vec3>());
        }
    };

    template<>
    struct var_type<glm::vec4> : var_defaults<"vec4", glm::vec4>
    {
        constexpr static inline char ctype_v = '4';
        constexpr static inline auto serialization_type_v = "4";
        template< typename T, auto CRC_V >
        constexpr static auto XCoreTextFile(T& FileStream, ::xproperty::any& Any)
        {
            if (Any.m_pType == nullptr || Any.m_pType->m_GUID != guid_v) Any.Reset<glm::vec4>();
            return FileStream.Field(CRC_V, "Value:?", Any.get<glm::vec4>());
        }
    };

    template<>
    struct var_type<glm::mat4> : var_defaults<"mat4", glm::mat4>
    {
        constexpr static inline char ctype_v = 'M';
        constexpr static inline auto serialization_type_v = "M";
        template< typename T, auto CRC_V >
        constexpr static auto XCoreTextFile(T& FileStream, ::xproperty::any& Any)
        {
            if (Any.m_pType == nullptr || Any.m_pType->m_GUID != guid_v) Any.Reset<glm::mat4>();
            return FileStream.Field(CRC_V, "Value:?", Any.get<glm::mat4>());
        }
    };

    template<>
    struct var_type<glm::quat> : var_defaults<"quat", glm::quat>
    {
        constexpr static inline char ctype_v = 'Q';
        constexpr static inline auto serialization_type_v = "Q";
        template< typename T, auto CRC_V >
        constexpr static auto XCoreTextFile(T& FileStream, ::xproperty::any& Any)
        {
            if (Any.m_pType == nullptr || Any.m_pType->m_GUID != guid_v) Any.Reset<glm::quat>();
            return FileStream.Field(CRC_V, "Value:?", Any.get<glm::quat>());
        }
    };
}
